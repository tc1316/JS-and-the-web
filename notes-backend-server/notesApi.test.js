...
describe('Notes class', () => {
  ...
  it('calls fetch to create new notes', async () => {
    const api = new NotesApi();
    let payload = fetch.mockResponseOnce(JSON.stringify({ "content": "Remember to reflect on my progress this week!" }))

    api.createNote(payload,(note) => {
      expect(note).toEqual({ "content": "Remember to reflect on my progress this week!" })
    } )
